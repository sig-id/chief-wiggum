...
agent_run() {
    ...
    if ! batch_coord_mark_complete "$batch_id" "$task_id" "$project_root"; then
    ...
    status=$(batch_coord_get_status "$batch_id" "$project_root")
    new_position=$(batch_coord_get_position "$batch_id" "$project_root")
