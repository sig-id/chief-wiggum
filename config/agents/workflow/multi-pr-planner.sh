...
source "$INDEXER_HOME/lib/scheduler/batch-coordination.sh"

# Main entry point
agent_run() {
    local worker_dir="$1"
    local project_root="$2"
    local max_turns="${INDEXER_MULTI_PR_PLANNER_MAX_TURNS:-${AGENT_CONFIG_MAX_TURNS:-50}}"
    ...
    # Set up context
    agent_setup_context "$worker_dir" "$project_root" "$project_root"
    ...
    while read -r task_json; do
        ...
        if [[ ! "$task_worker_dir" = /* ]]; then
            task_worker_dir="$project_root/$task_worker_dir"
        fi
...
"
    ...
    # Build user prompt
    local user_prompt
    user_prompt=$(_get_user_prompt "$batch_file" "$task_contexts" "$project_root")

    # Use step ID from pipeline for log naming
    local step_id="${INDEXER_STEP_ID:-planner}"
    local log_file
    ...
    mkdir -p "$project_root/.indexer/plans"
    while read -r task_json; do
        local tid
        tid=$(echo "$task_json" | jq -r '.task_id')
        rm -f "$project_root/.indexer/plans/resolution-plan-${tid}.md"
    done < <(jq -c '.tasks[]' "$batch_file")

    log "Running planner (max $max_turns turns)..."

    # Run one-shot Claude call
    run_agent_once "$project_root" "$system_prompt" "$user_prompt" "$log_file" "$max_turns"
    local agent_exit=$?
    ...
    # Distribute plans from .indexer/plans/ to worker directories
    ...
    while read -r task_json; do
        ...
        local plan_file="$project_root/.indexer/plans/resolution-plan-${task_id}.md"
    ...
    if [ "$plans_created" -gt 0 ]; then
        ...
        _setup_batch_coordination "$batch_file" "$log_file" "$project_root"
    fi
...
For each task, write a resolution plan file:
    ...
    .indexer/plans/resolution-plan-{TASK-ID}.md
...
_get_user_prompt() {
    ...
    while read -r task_json; do
        local tid
        tid=$(echo "$task_json" | jq -r '.task_id')
        task_file_instructions+="- Write ${tid}'s plan to \`.indexer/plans/resolution-plan-${tid}.md\`
"
    done < <(jq -c '.tasks[]' "$batch_file")
...
#
# Creates:
#   - .indexer/batches/{batch_id}/coordination.json : Shared coordination file
...
_setup_batch_coordination() {
    ...
    batch_coord_init "$batch_id" "$task_order_list" "$project_root"
