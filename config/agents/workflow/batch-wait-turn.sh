...
BATCH_POLL_INTERVAL="${INDEXER_BATCH_POLL_INTERVAL:-5}"

# Maximum wait time before giving up (seconds) - default 30 minutes
BATCH_MAX_WAIT="${INDEXER_BATCH_MAX_WAIT:-1800}"
...
agent_run() {
    ...
    # Set up context
    agent_setup_context "$worker_dir" "$worker_dir/workspace" "$project_root"
    ...
    while true; do
        ...
        local turn_result=0
        batch_coord_is_my_turn "$batch_id" "$task_id" "$project_root" || turn_result=$?
        ...
        local current_pos
        current_pos=$(batch_coord_get_position "$batch_id" "$project_root" 2>/dev/null || echo "?")
