...
#
# Workspace cleanup is NOT done here — it is handled by `indexer service stop`
...
agent_source_core
source "$INDEXER_HOME/lib/git/git-operations.sh"
source "$INDEXER_HOME/lib/git/worktree-helpers.sh"
...
agent_run() {
    ...
    if [[ "$gate_result" == "PASS" ]]; then
        if _handle_pass "$workspace" "$worker_dir" "$project_root"; then
            ...
            _reset_workspace_to_main "$workspace" "$project_root"
        else
...
_handle_pass() {
    ...
    local task_id="${INDEXER_TASK_ID:-autofix}"
    local commit_msg
    commit_msg=$(_build_commit_msg "$task_id" "$audit_report")

    if [[ -n "$audit_report" ]] && [[ -f "$audit_report" ]]; then
        local reserve_rc=0
        _autofix_reserve_audit "$audit_report" "$worker_dir" "$project_root" "$commit_msg" || reserve_rc=$?
    ...
    if [[ "$commit_exit" -ne 0 ]]; then
        ...
        _autofix_mark_audit_state "$audit_report" "$project_root" "commit_failed" "" "" \
    ...
    _push_per_cycle_pr "$workspace" "$commit_sha" "$commit_msg" "$worker_dir" "$project_root" "$audit_report"
}
...
#   project_root - Project directory (git root)
...
_autofix_dedupe_file() {
    ...
    local indexer_dir="${INDEXER_DIR:-$project_root/.indexer}"
    echo "$indexer_dir/autofix/dedupe.json"
}
...
_autofix_remote_duplicate_exists() {
    ...
    [[ "${INDEXER_AUTOFIX_REMOTE_DEDUPE:-true}" == "false" ]] && return 1
    command -v gh &>/dev/null || return 1

    local gh_timeout="${INDEXER_GH_TIMEOUT:-30}"
    local json count
...
_autofix_reserve_audit() {
    ...
    state_file=$(_autofix_dedupe_file "$project_root")
    ...
    metadata=$(_autofix_audit_metadata_json "$audit_report")
    now=$(epoch_now)
    ttl="${INDEXER_AUTOFIX_DEDUPE_TTL_SECONDS:-604800}"
    [[ "$ttl" =~ ^[0-9]+$ ]] || ttl=604800
    cutoff=$((now - ttl))
    lease_ttl="${INDEXER_AUTOFIX_RESERVATION_TTL_SECONDS:-21600}"
    ...
    if _autofix_remote_duplicate_exists "$fingerprint" "$title"; then
        _autofix_mark_audit_state "$audit_report" "$project_root" "duplicate_remote" "" ""
        return 2
    fi
...
_push_per_cycle_pr() {
    ...
    local commit_msg="$3"
    local worker_dir="$4"
    local project_root="${5:-${PROJECT_DIR:-}}"
    ...
    if [[ "$branch_exit" -ne 0 ]]; then
        ...
        _fallback_push "$workspace" "$task_branch" "$commit_msg" "$worker_dir" "$project_root" "$audit_report"
        return
    fi
    ...
    if [[ "$pick_exit" -ne 0 ]]; then
        ...
        _autofix_mark_audit_state "$audit_report" "$project_root" "cherry_pick_failed" "$cycle_branch" "" \
            || log_warn "Failed to update autofix dedupe state"
        return 1
    fi
    ...
    if ! _with_backoff 3 "Push $cycle_branch" \
        ...
        _autofix_mark_audit_state "$audit_report" "$project_root" "push_failed" "$cycle_branch" "" \
            || log_warn "Failed to update autofix dedupe state"
        return 1
    fi
    ...
    if _with_backoff 3 "Create PR for $cycle_branch" \
        ...
        _autofix_mark_audit_state "$audit_report" "$project_root" "pr_created" "$cycle_branch" "$AUTOFIX_LAST_PR_URL" \
            || log_warn "Failed to update autofix dedupe state"
    else
        log_warn "PR creation failed — branch was pushed, PR can be created manually"
        AUTOFIX_PASS_ACTION="branch_pushed"
        _autofix_mark_audit_state "$audit_report" "$project_root" "branch_pushed" "$cycle_branch" "" \
            || log_warn "Failed to update autofix dedupe state"
    fi
...
_make_cycle_branch_name() {
    ...
    epoch=$(epoch_now)
    local worker_slug
    worker_slug=$(printf '%s' "${INDEXER_TASK_ID:-autofix}-$$" \
        | tr '[:upper:]' '[:lower:]' \
...
_fallback_push() {
    ...
    if ! _with_backoff 3 "Fallback push $branch" \
        ...
        if ! _with_backoff 2 "Force push $branch" \
            ...
            _autofix_mark_audit_state "$audit_report" "$project_root" "push_failed" "$branch" "" \
                || log_warn "Failed to update autofix dedupe state"
            return 1
        fi
    fi
    ...
    if _with_backoff 3 "Create PR for $branch" \
        ...
        AUTOFIX_PASS_ACTION="pr_created"
        _autofix_mark_audit_state "$audit_report" "$project_root" "pr_created" "$branch" "$AUTOFIX_LAST_PR_URL" \
            || log_warn "Failed to update autofix dedupe state"
    else
        log_warn "PR creation failed — branch was pushed, PR can be created manually"
        AUTOFIX_PASS_ACTION="branch_pushed"
        _autofix_mark_audit_state "$audit_report" "$project_root" "branch_pushed" "$branch" "" \
            || log_warn "Failed to update autofix dedupe state"
    fi
}
...
_build_pr_body() {
    ...
    if [[ -n "$fingerprint" ]]; then
        printf '<!-- indexer-autofix-fingerprint: %s -->\n\n' "$fingerprint"
    fi
