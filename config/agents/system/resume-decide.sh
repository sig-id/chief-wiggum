...
_load_pipeline_config() {
    local project_root="${1:-$INDEXER_HOME}"

    # Try to resolve and load pipeline config
    local pipeline_path
    pipeline_path=$(pipeline_resolve "$project_root" "" "${INDEXER_PIPELINE:-}")
...
_get_agent_mode() {
    ...
    local md_file="$INDEXER_HOME/lib/agents/${agent_path}.md"
...
agent_run() {
    ...
    _load_pipeline_config "$project_root"
    ...
    agent_setup_context "$worker_dir" "$worker_dir" "$project_root"
    ...
    local step_id="${INDEXER_STEP_ID:-resume-decide}"
    local run_epoch
    run_epoch=$(epoch_now)
...
Output the <decision> tag NOW. No explanation, no other text."
    ...
    local step_id="${INDEXER_STEP_ID:-resume-decide}"
    local run_id="${INDEXER_RUN_ID:-default}"
    local backup_log
