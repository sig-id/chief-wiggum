...
source "$INDEXER_HOME/lib/worker/git-state.sh"
...
agent_run() {
    ...
    local max_iterations="${INDEXER_COMMENT_FIX_MAX_ITERATIONS:-${AGENT_CONFIG_MAX_ITERATIONS:-10}}"
    local max_turns="${INDEXER_COMMENT_FIX_MAX_TURNS:-${AGENT_CONFIG_MAX_TURNS:-30}}"
    ...
    if [ ! -f "$comments_file" ]; then
        ...
        log_error "Run 'indexer pr comments <pattern> sync' first"
        agent_log_complete "$worker_dir" 1 "$start_time"
    ...
    if [ "$comment_count" -eq 0 ]; then
        ...
        agent_setup_context "$worker_dir" "$workspace" "$project_root"
    ...
    local session_prefix="${INDEXER_STEP_ID:-pr-fix}"
    ...
    local commit_sha=""
    local auto_commit="${INDEXER_AUTO_COMMIT_AFTER_FIX:-${AGENT_CONFIG_AUTO_COMMIT:-true}}"
...
_commit_and_push_fixes() {
    ...
    local commit_msg="${INDEXER_TASK_ID:+${INDEXER_TASK_ID}: }${INDEXER_STEP_ID:-pr-fix} - pr-comment-fix"
